...
Let's get started with building the project. Open a terminal and ```cd``` into the root of the solution. Then run the following to start the Docker build of your cron job: 
``` bash
...
```
Make sure to replace the text ```my-registry-host``` with your registry, or remove it if you're using docker hub. Now, replace the line in your ```kube.yml``` file with the registry reference above:
``` yaml
kind: CronJob
metadata:
  name: mycronjob
